...
def pre_process(input_csv_path):
    """
...
def extract_features(X, HRV_features):
    """
...
def predict(feature_df, model):
    prediction = model.predict(feature_df)
    return prediction


def main():
    model = load_classifier('model/tf')

    t1 = timer()
    X, HRV_features = pre_process('datas/vid_s8_T1.csv')
    t2 = timer()
    feature_df = extract_features(X, HRV_features)
    t3 = timer()
    prediction = predict(feature_df, model)
    t4 = timer()
    print(prediction)
    print(f'Timings\n'
