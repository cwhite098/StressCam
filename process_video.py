        ...
        with mp_face_mesh.FaceMesh(static_image_mode=True, max_num_faces=1,
            ...
            with mp_holistic.Holistic() as holistic:
                ...
                while fvs.running():
                    ...
                    pointer_frame = HT.get_angular_position(detection.landmark, display_frame, display=False)
                    #display_frame = ET.draw_circles(display_frame)
                    RR.vid_feed_per_frame_analysis(image, fps, frame_num, mp_holistic, holistic)

                    # Finished processing, record frame time
                    new_frame_time = time.time()
                    disp_fps = 1 / (new_frame_time - prev_frame_time)
